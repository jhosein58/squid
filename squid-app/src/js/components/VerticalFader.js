...
export class VerticalFader extends BaseComponent {
  ...
  set value(newValue) {
    ...
    if (this._value.toFixed(4) !== clampedValue.toFixed(4)) {
      ...
      if (this._externalCallback) this._externalCallback(this._value);
    }
  }
  ...
  drawOverlay(ctx) {
    ...
    const boxHeight = parseInt(font, 10) + paddingY * 2;
    ...
    ctx.fill();
    if (border) {
      ctx.strokeStyle = border.split(" ")[border.split(" ").length - 1];
      ctx.lineWidth = parseFloat(border);
      ctx.stroke();
    }
    ctx.fillStyle = color;
    ctx.textAlign = "center";
